...
\label{eq:zf_power_constraint}
    \sum_{k=1}^{K} \sum_{i=1}^{N_S} \, q_{k, i} \, \big((\mathbf{H} \mathbf{H}^H)^{-1}\big)_{n,\,n} \leq P_t \qquad \text{with} \quad n = (k-1)\,N_S + i.
\end{equation}
Let us introduce the substitution
\begin{equation}
    p_n = \gamma_n^{-1} \, q_{k, i} \quad \text{with} \quad \gamma^{-1}_n = \big((\mathbf{H} \mathbf{H}^H)^{-1}\big)_{n,\,n},
\end{equation}
...
\begin{algorithmic}[1]
    ...
    \Statex $P_t = P_t, \quad \alpha_n = 1, \quad \text{and} \quad  \gamma_n = \big(N_0 \, \big((\mathbf{H} \mathbf{H}^H)^{-1}\big)_{n,\,n}\big)^{-1}.$
    
    \vspace{0.4\baselineskip}
    \State Compute the entries of the scaling matrix as
    \Statex $q_{k, i} = p_n \, \big((\mathbf{H} \mathbf{H}^H)^{-1}\big)_{n,\,n} \quad \text{with} \quad n = (k-1)\,N_S + i, \quad \text{for  } k = 1, \dots, K \text{  and  } i = 1, \dots, N_S.$
...
\section{Zero-Forcing Precoding with an Heuristic LSV Combiner}
...
This comparison indicates that, on average, adding two transmit antennas at the \ac{BS} and switching from \ac{SVD} precoding in \ac{SU}-\ac{MIMO} to \ac{BD} precoding in \ac{MU}-\ac{MIMO} enables serving an additional two-stream user with the same reliability only a minor per-user throughput penalty.
...
\label{eq:mu_mimo_wsr_problem}
    \underset{\{\mathbf{F}_k\}}{\max} \;\; \sum_{k=1}^{K} w_k \, R_k \qquad \text{s.t.} \quad \sum_{k=1}^{K} \mathrm{tr}\!\left(\mathbf{F}_k \mathbf{F}_k^H\right) \leq P_t.
\end{equation}
...
\label{eq:effective_noise_covariance}
    \mathbf{C}_{\tilde{n}_k} = N_0 \, \mathbf{I}_{N_R} + \sum_{k' \neq k} \mathbf{H}_k \mathbf{F}_{k'} \mathbf{F}_{k'}^H \mathbf{H}_k^H.
\end{equation}
Consequently, the achievable rate of \ac{UT}~$k$ can be written similarly to the \ac{SU} case as
\begin{equation}
\label{eq:mu_mimo_wmmse_rate}
    R_k = \log_2 \det\!\Big(\mathbf{I}_{N_S} + \mathbf{F}_k^H \mathbf{H}_k^H \mathbf{C}_{\tilde{n}_k}^{-1} \mathbf{H}_k \mathbf{F}_k\Big).
\end{equation}
...
\label{eq:mu_mimo_mmse_combiner}
    \mathbf{G}_k^{\mathrm{MMSE}} = \mathbf{F}_k^H \mathbf{H}_k^H \big(\mathbf{H}_k \mathbf{F}_k \mathbf{F}_k^H \mathbf{H}_k^H + \mathbf{C}_{\tilde{n}_k}\big)^{-1}.
\end{equation}
...
\label{eq:mu_mimo_mse_matrix}
    \mathbf{E}_k 
    = \mathbb{E}\!\left[(\mathbf{a}_k - \mathbf{z}_k)(\mathbf{a}_k - \mathbf{z}_k)^H\right]
    = \big(\mathbf{I}_{N_S} + \mathbf{F}_k^H \mathbf{H}_k^H \mathbf{C}_{\tilde{n}_k}^{-1} \mathbf{H}_k \mathbf{F}_k\big)^{-1}.
\end{equation}
...
It is the multi-user analogue of the rate--MSE link already established in the generalized \ac{WMMSE} design of Section~\ref{section:generalized_wmmse_design}.
\medskip
...
\label{eq:mu_mimo_wmmse_problem}
    \min_{\{\mathbf{F}_k\}} \; \sum_{k=1}^{K} \mathrm{tr}(\mathbf{W}_{\mathrm{e}, \,k} \, \mathbf{E}_k) \qquad \mathrm{s.\,t.} \quad \sum_{k=1}^{K} \mathrm{tr}(\mathbf{F}_k \mathbf{F}_k^H) \leq P_t,
\end{equation}
...
\label{eq:mu_mimo_wmmse_precoder_update}
    \bar{\mathbf{F}} = \Big(\mathbf{H}^H \mathbf{G}^H \mathbf{W} \mathbf{G} \mathbf{H} + \mathrm{tr}(\mathbf{W} \mathbf{G} \mathbf{G}^H) \, \frac{N_0}{P_t} \, \mathbf{I}_{N_T}\Big)^{-1} \mathbf{H}^H \mathbf{G}^H \mathbf{W},
\end{equation}
followed by the rescaling $\mathbf{F} = \beta \, \bar{\mathbf{F}}$ with $\beta^2 = P_t / \mathrm{tr}(\bar{\mathbf{F}} \bar{\mathbf{F}}^H)$ to enforce the total power constraint with equality.
...
\begin{algorithmic}[1]
    ...
    \Repeat
        ...
        \Statex \hspace{\algorithmicindent} $\mathbf{G}_k = \mathbf{F}_k^H \mathbf{H}_k^H \big(\mathbf{H}_k \mathbf{F}_k \mathbf{F}_k^H \mathbf{H}_k^H + \mathbf{C}_{\tilde{n}_k}\big)^{-1} \quad \text{for} \quad k = 1, \dots, K.$
        ...
        \Statex \hspace{\algorithmicindent} $\bar{\mathbf{F}} = \Big(\mathbf{H}^H \mathbf{G}^H \mathbf{W} \mathbf{G} \mathbf{H} + \mathrm{tr}(\mathbf{W} \mathbf{G} \mathbf{G}^H) \, \frac{N_0}{P_t} \, \mathbf{I}_{N_T}\Big)^{-1} \mathbf{H}^H \mathbf{G}^H \mathbf{W}$
        \Statex \hspace{\algorithmicindent} $\mathbf{F} = \beta \, \bar{\mathbf{F}} \quad \text{with} \quad \beta^2 = P_t / \mathrm{tr}(\bar{\mathbf{F}} \bar{\mathbf{F}}^H)$
...
In all three cases, the end-to-end processing transforms the original \ac{MU}--\ac{MIMO} channel into parallel scalar Gaussian subchannels, as illustrated in Figure~\ref{fig:mu_mimo_channel_decomposistion}.
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \caption[Channel decomposistion of a \ac{MU}--\ac{MIMO} system]{A block diagram of the original \ac{MU}--\ac{MIMO} channel (a) and its decomposition into parallel Gaussian \ac{SISO} subchannels (b), as achieved by the \ac{ZF}--type precoding techniques.}
    \label{fig:mu_mimo_channel_decomposistion}
\end{figure}

Figure~\ref{fig:mu_mimo_eff_subchannel_gain_distributions_8x2x4} reports the marginal \acp{pdf} for a single user in an system with $8$ transmit antennas at the \ac{BS} and $4$ two-antenna \acp{UT}, obtained through Monte--Carlo simulations.
...
For pure \ac{ZF}, the effective subchannel gains are given by $g_n=\big((\mathbf{H}\mathbf{H}^H)^{-1}\big)_{n,n}^{-1}$ and, under an \ac{i.i.d.} Rayleigh fading model, their distribution follows from the inverse-Wishart model~\cite{wishart_matrices_1, wishart_matrices_0}.
In particular, each $g_n$ is Gamma-distributed with shape parameter $\alpha = N_T-KN_R+1$ and rate parameter $\beta=1$.
For \ac{ZF} with an heuristic \ac{LSV} combiner and for \ac{BD} precoding, no analytical distribution is derived..
...
\begin{figure}[!hbt]
    ...
    \end{subfigure}
    \caption[Achievable sum rate comparison of the different precoding techniques in two \ac{MU}--\ac{MIMO} systems]
    {Comparison of the achievable sum rate for the considered \ac{MU} linear precoding techniques in two fully loaded \ac{MU}--\ac{MIMO} systems.
    The results are averaged over at least $1000$ \ac{i.i.d.} Rayleigh channel realizations.}
...
A notable exception to the above ranking occurs at very high \ac{SNR} in the $8 \times \{4,4\}$ configuration (Figure~\ref{fig:mu_mimo_sum_rate_comparison_8x4x2}), where \ac{BD} eventually overtakes \ac{WMMSE}.
...
The choice between these techniques in practice therefore involves a tradeoff between achievable \ac{SR}, computational complexity, and signaling overhead.
