...
It will be shown that the optimal precoder and combiner under this criterion likewise decouple the channel into parallel eigen subchannels, regardless of the specific choice of error weights.
...
The \ac{WMSE} matrix is defined as
\begin{equation}
...
Solving these coupled matrix equations yields the optimal linear precoder and combiner.
...
\end{equation}
In other words, the optimal precoder and combiner diagonalize the \ac{SU}-\ac{MIMO} transfer matrix $\mathbf{T}$, and hence decouple the channel into parallel eigen subchannels, regardless of the specific choice of error weights. This result is similar to the capacity-achieving precoder and combiner from Section~\ref{subsection:optimal_precoder_design}, who also decompose the channel matrix $\mathbf{H}$.
...
It was already shown in Section~\ref{subsection:optimal_precoder_design} that the capacity of the \ac{SU}-\ac{MIMO} channel can be achieved by a linear precoder and combiner that diagonalize the transfer matrix and a waterfilling power allocation strategy.
...
The above equation is obtained by choosing the error weight matrix as $\mathbf{W}_{\mathrm{e}} = \mathbf{\Lambda}$ in~\eqref{eq:solution_generalized_design_precoder_phi}. It is shown in~\cite{su_mimo_joint_precoder_decoder_design} that the optimal precoder and combiner that maximize the information rate are given by~\eqref{eq:solution_generalized_design_precoder} and~\eqref{eq:solution_generalized_design_combiner} for this particular choice of error weights.
