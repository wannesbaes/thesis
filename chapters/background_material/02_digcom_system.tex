...
\label{eq:mutual_information_su}
\end{equation}
...
\label{eq:capacity}
\end{equation}
Typically, the maximization is subject to the transmit power constraint, as formulated in \eqref{eq:Pt_constraint_su}.
...
\label{eq:MI_Gaussian_channel}
\end{equation}
The capacity then results from maximizing \eqref{eq:MI_Gaussian_channel} over all positive semidefinite covariance matrices $\mathbf{C}_{\mathrm{x}}$ that satisfy the transmit power constraint:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\label{eq:capacity_Gaussian_channel}
\end{equation}
...
\begin{figure}[!h]
    \centering
    \caption{Example of a rate region $\mathcal{R}$ and the sum-capacity $C_{\text{sum}}$ for two users in a MU-MIMO system.}
...
The sum-capacity corresponds to the special case where all weights are equal to one. The maximization in both cases is typically subject to the total transmit power constraint \eqref{eq:Pt_constraint_mu}.

The sum-capacity of the \ac{MU}-\ac{MIMO} Gaussian channel is derived in~\cite{viswanath_2002, viswanath_2003, yu_2004} by exploiting the uplink--downlink duality: the downlink broadcast channel and the uplink multiple-access channel yield
...
In the uplink channel, all $K$ \acp{UT} transmit simultaneously, with \ac{UT}\textsubscript{k} sending a vector $\mathbf{x}_k \in \mathbb{C}^{N_R}$ to the \ac{BS}.
The \ac{MI} between the transmitted vectors $\mathbf{x}_1, \dots, \mathbf{x}_K$ and the received vector $\mathbf{y}$, given input covariance matrices $\mathbf{C}_{\mathrm{x},k}$, is defined as
\begin{equation}
    I(\mathbf{x}_1, \dots, \mathbf{x}_K; \mathbf{y})
    = \mathbb{E}\!\left[ \log_2\!\left( \frac{p(\mathbf{y} \mid \mathbf{x}_1, \dots, \mathbf{x}_K)}{p(\mathbf{y})} \right) \right],
\label{eq:mutual_information_mu}
\end{equation}
where the expectation is taken with respect to the joint \ac{pdf} of $(\mathbf{x}_1, \dots, \mathbf{x}_K)$ and $\mathbf{y}$, and $p(\mathbf{y})$ is the marginal \ac{pdf} of $\mathbf{y}$.
It is further obtained as
\begin{equation}
    I(\mathbf{x}_1, \dots, \mathbf{x}_K; \mathbf{y})
    = \log_2\!\left(\det\!\left(\mathbf{I}_{N_T} + \mathbf{C}_{\mathrm{n}}^{-1} \sum_{k=1}^{K} \mathbf{H}_k\,\mathbf{C}_{\mathrm{x},k}\,\mathbf{H}_k^\mathrm{H} \right)\right).
\label{eq:mutual_information_uplink}
\end{equation}
The uplink \ac{SR} is achievable if and only if the sum of the \ac{IBR}s of all \acp{UT} is strictly smaller than the \ac{MI} as given in~\eqref{eq:mutual_information_uplink}. Therefore, the sum-capacity of the downlink channel is obtained by maximizing \eqref{eq:mutual_information_uplink} over all positive semidefinite covariance matrices satisfying the total transmit power constraint and applying the duality result:
\begin{equation}
\begin{aligned}
    C_{\text{sum}} =& \max_{\mathbf{C}_{\mathrm{x},1}, \dots, \mathbf{C}_{\mathrm{x},K}} \; \log_2\!\left(\det\!\left(\mathbf{I}_{N_T} + \mathbf{C}_{\mathrm{n}}^{-1} \sum_{k=1}^{K} \mathbf{H}_k\,\mathbf{C}_{\mathrm{x},k}\,\mathbf{H}_k^\mathrm{H} \right)\right) \\
    &\text{s.t.} \quad \sum_{k=1}^{K} \mathrm{tr}\!\left(\mathbf{C}_{\mathrm{x},k}\right) \leq P_t \quad \text{and} \quad \mathbf{C}_{\mathrm{x},k} \succeq 0, \quad \forall k = 1, \dots, K.
\end{aligned}
\label{eq:sum_capacity_Gaussian_channel}
\end{equation}
