...
In this chapter, we consider the downlink \ac{MU}-\ac{MIMO} system introduced in Section~\ref{subsection:mu_mimo_system_model}.
...
\begin{figure}[!htbp]
    ...
    \end{subfigure}
    \caption[Visualization of \acs{ZF} precoding (dual-antenna \acs{BS} \& two single-antenna \acsp{UT})]
    {Visualization of \acs{ZF} precoder for a \acs{MU}-\acs{MIMO} system with two single-antenna \acsp{UT} and a dual-antenna \acs{BS} under (a) favorable and (b) unfavorable channel conditions.
...
\begin{figure}[!htbp]
    ...
    \end{subfigure}
    \caption[Visualization of \acs{ZF} precoding (triple-antenna \acs{BS} \& two single-antenna \acsp{UT})]
    {Visualization of \acs{ZF} precoder for a \acs{MU}-\acs{MIMO} system with two single-antenna \acsp{UT} and a triple-antenna \acs{BS}.
...
A practical extension is therefore to equip each \ac{UT} with a heuristic \acf{LSV} combiner, while keeping the precoder design at the \ac{BS} based on \ac{ZF}.
The key idea is to improve the virtual channel gains seen by the \ac{BS} without increasing the signaling overhead.
...
The zero inter-user interference condition requires that the precoder $\mathbf{F}_k$ intended for \ac{UT} $k$ is located in the null space of the channel matrices of all other users.
...
The signals intended for the other users act, under Gaussian signaling, as additional Gaussian noise, such that
\begin{equation}
...
\end{equation}
Comparing~\eqref{eq:mu_mimo_wmmse_rate} and~\eqref{eq:mu_mimo_mse_matrix}, the achievable rate of \ac{UT}~$k$ can be expressed directly in terms of its \ac{MSE} matrix as~\cite{WMMSE_precoding}
\begin{equation}
...
\end{equation}
where each $\mathbf{W}_{\mathrm{e}, k} \in \mathbb{C}^{N_S \times N_S}$ is a positive-definite weight matrix associated with \ac{UT}~$k$ and $\mathbf{E}_k$ is the \ac{MSE} matrix in~\eqref{eq:mu_mimo_mse_matrix}.
...
\begin{algorithmic}[1]
    ...
    \medskip
    \State Initialize the precoders:
    ...
    \Repeat
        ...
        \vspace{0.2\baselineskip}
        \State Update the combiners:
        \Statex \hspace{\algorithmicindent}
        $\mathbf{G}_k = \mathbf{F}_k^{\mathsf{H}} \mathbf{H}_k^{\mathsf{H}} \big(\mathbf{H}_k \mathbf{F}_k \mathbf{F}_k^{\mathsf{H}} \mathbf{H}_k^{\mathsf{H}} + \mathbf{C}_{\tilde{n}_k}\big)^{-1}$ 
        \hspace{0.1cm} with $\mathbf{C}_{\tilde{n}_k}$ from~\eqref{eq:effective_noise_covariance}, 
        \hspace{0.05cm} for $k = 1, \dots, K.$

        \vspace{0.2\baselineskip}
        \State Update the weights:
        \Statex \hspace{\algorithmicindent} $\mathbf{W}_k = \mathbf{E}_k^{-1}$
        \hspace{0.1cm} with $\mathbf{E}_k$ from~\eqref{eq:mu_mimo_mse_matrix} and $\mathbf{C}_{\tilde{n}_k}$ from~\eqref{eq:effective_noise_covariance},
        \hspace{0.05cm} for $k = 1, \dots, K$.

        \vspace{0.2\baselineskip}
        \State Update the compound precoder:
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \caption[Channel decomposition of a \acs*{MU}-\acs*{MIMO} system]
    {A block diagram of the original \acs*{MU}-\acs*{MIMO} channel (a) and its decomposition into parallel Gaussian \acs*{SISO} subchannels (b), as achieved by the \acs*{ZF}-type precoding techniques.}
...
\begin{figure}[htb!]
    ...
    \caption[\acs{pdf} of the effective subchannel gains in a $8 \times \{2, 2, 2, 2\} \;$ \acs*{MU}-\acs*{MIMO} system]
    {The marginal \acsp{pdf} of the effective subchannel gains for a single \ac{UT} in a $8 \times \{2, 2, 2, 2\} \;$ \ac{MU}-\ac{MIMO} system, compared between the different \acs{ZF}-type precoding techniques.
    The expected value is indicated by a vertical dashed line, and the shaded area around the expected value corresponds to one standard deviation.}
...
\begin{figure}[!hbt]
    ...
    \end{subfigure}
    \caption[Achievable \acs{SR} comparison for the different precoders]
...
For pure \ac{ZF}, the two curves are identical: since no receiver combining is applied and the total number of receive antennas is the same, the achievable \ac{SR} depends only on the compound channel and is unaffected by how the antennas are grouped across \acp{UT}.
...
The benefit of concentrating antennas is more pronounced for \ac{BD} than for \ac{ZF}+\ac{LSV}, which reflects the fact that the coordinated \ac{BD} design makes better use of the per-user spatial degrees of freedom than the non-coordinated \ac{LSV} heuristic.
...
In the next chapters, performance evaluations are carried out using the \ac{ZF}, \ac{BD} and \ac{WMMSE} precoder.
