...
When multiple receivers are served simultaneously by a single multi-antenna transmitter, the system is referred to as a downlink \ac{MU} \ac{MIMO} system. In this context, the transmitter is called the \ac{BS} and the individual receivers are referred to as \acp{UT}. A block diagram is shown in Fig.~\ref{fig:mu_mimo_block_diagram}.
...
The block-diagonal structure of $\mathbf{G}$ indicates that each \ac{UT} processes only its own received signal vector, i.e., no cooperation is assumed between different \acp{UT}.
