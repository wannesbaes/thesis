...
\begin{figure}[hb]
    \centering
    \makebox[\textwidth][c]{\resizebox{\textwidth}{!}{\includegraphics{figures/background_material/02_communication_systems/out/digcom_system_block_diagram.pdf}}}
...
These decision variables are processed by the detector to obtain estimates of the originally transmitted symbols, which are then mapped back to coded bits.
...
Compared to a single-antenna link, the multiple antennas provide additional spatial degrees of freedom that can be exploited to increase data throughput, improve reliability, or achieve a trade-off between the two.
...
\begin{figure}[!ht]
    \centering
    \makebox[\textwidth][c]{\resizebox{1.2\textwidth}{!}{\includegraphics{figures/background_material/02_communication_systems/out/su_mimo_block_diagram_detailed.pdf}}}
...
The noise vector is \ac{CS} complex Gaussian distributed, i.e.\ $\mathbf{n} \sim \mathcal{CN}(\mathbf{0}, N_0 \, \mathbf{I}_{N_R})$, such that its entries are spatially independent, each with an expected power of $N_0$.
...
\end{equation}
The data symbols are assumed to be zero-mean, mutually uncorrelated, and normalized, such that $ \mathbb{E}\!\left[\mathbf{a}\mathbf{a}^{\mathsf{H}}\right] = \mathbf{I}_{N_S} $.
...
\label{eq:su_mimo_power_constraint}
    \mathbb{E}\!\left[\|\mathbf{x}\|^2\right] = \mathrm{tr}\!\left(\mathbf{F}^{\mathsf{H}} \, \mathbf{F}\right) \leq P_T.
\end{equation}
\begin{comment}
\begin{subequations}
\begin{align}
    ...
    &= \mathbb{E}\!\left[ \mathbf{x}^{\mathsf{H}} \mathbf{x} \right] \\
    &= \mathbb{E}\!\left[ \left(\mathbf{F} \mathbf{a}\right)^{\mathsf{H}} \left(\mathbf{F} \mathbf{a}\right) \right] \\
    &= \mathbb{E}\!\left[ \mathbf{a}^{\mathsf{H}} \mathbf{F}^{\mathsf{H}} \mathbf{F} \mathbf{a} \right] \\
    &= \mathbb{E}\!\left[ \text{Tr}\!\left[ \mathbf{a}^{\mathsf{H}} \mathbf{F}^{\mathsf{H}} \mathbf{F} \mathbf{a} \right] \right] \\
    &= \mathbb{E}\!\left[ \text{Tr}\!\left[ \mathbf{F}^{\mathsf{H}} \mathbf{F} \, \mathbf{a} \mathbf{a}^{\mathsf{H}} \right] \right] \\
    &= \text{Tr}\!\left[ \mathbb{E}\!\left[ \mathbf{F}^{\mathsf{H}} \mathbf{F} \, \mathbf{a} \mathbf{a}^{\mathsf{H}} \right]  \right] \\
    &= \text{Tr}\!\left[ \mathbf{F}^{\mathsf{H}} \mathbf{F} \, \mathbb{E}\!\left[ \mathbf{a} \mathbf{a}^{\mathsf{H}} \right]  \right] \\
    &= \text{Tr}\!\left[ \mathbf{F}^{\mathsf{H}} \mathbf{F} \, \mathbf{I}_{N_S} \right] \\
    &= \text{Tr}\!\left[ \mathbf{F}^{\mathsf{H}} \mathbf{F} \right] \\
...
When multiple receivers are served simultaneously by a single multi-antenna transmitter, the system is referred to as a downlink \acf{MU} \ac{MIMO} system. In this context, the transmitter is called the \ac{BS} and the individual receivers are referred to as \acp{UT}. A block diagram is shown in Fig.~\ref{fig:mu_mimo_block_diagram}.

\begin{figure}[!ht]
    \centering
    \makebox[\textwidth][c]{\resizebox{0.8\textwidth}{!}{\includegraphics{figures/background_material/02_communication_systems/out/mu_mimo_block_diagram.pdf}}}
...
\label{eq:mu_mimo_power_constraint}
    \mathbb{E}\!\left[\|\mathbf{x}\|^2\right] = \sum_{k=1}^{K} \mathrm{tr}\!\left(\mathbf{F}_k^{\mathsf{H}} \, \mathbf{F}_k\right) \leq P_T.
\end{equation}
\begin{comment}
\begin{subequations}
\begin{align}
    ...
    &\geq \mathbb{E}\!\left[ \left\|\mathbf{x}\right\|^2 \right] \\
    &= \mathbb{E}\!\left[ \mathbf{x}^{\mathsf{H}} \mathbf{x} \right] \\
    &= \mathbb{E}\!\left[ \left(\sum_{k=1}^{K} \mathbf{F}_k \mathbf{a}_k\right)^{\mathsf{H}} \left(\sum_{k'=1}^{K} \mathbf{F}_{k'} \mathbf{a}_{k'}\right) \right] \\
    &= \mathbb{E}\!\left[ \sum_{k=1}^{K} \left(\mathbf{F}_k \mathbf{a}_k\right)^{\mathsf{H}} \left(\sum_{k'=1}^{K} \mathbf{F}_{k'} \mathbf{a}_{k'}\right) \right] \\
    &= \mathbb{E}\!\left[ \sum_{k=1}^{K} \sum_{k'=1}^{K} \left(\mathbf{F}_k \mathbf{a}_k \right)^{\mathsf{H}} \mathbf{F}_{k'} \mathbf{a}_{k'} \right] \\
    &= \mathbb{E}\!\left[ \sum_{k=1}^{K} \sum_{k'=1}^{K} \mathbf{a}_k^{\mathsf{H}} \mathbf{F}_k^{\mathsf{H}} \mathbf{F}_{k'} \mathbf{a}_{k'} \right] \\
    &= \mathbb{E}\!\left[ \sum_{k=1}^{K} \sum_{k'=1}^{K} \text{Tr}\!\left[ \mathbf{a}_k^{\mathsf{H}} \mathbf{F}_k^{\mathsf{H}} \mathbf{F}_{k'} \mathbf{a}_{k'} \right] \right] \\
    &= \mathbb{E}\!\left[ \sum_{k=1}^{K} \sum_{k'=1}^{K} \text{Tr}\!\left[ \mathbf{F}_k^{\mathsf{H}} \mathbf{F}_{k'} \, \mathbf{a}_{k'} \mathbf{a}_k^{\mathsf{H}} \right] \right] \\
    &= \sum_{k=1}^{K} \sum_{k'=1}^{K} \text{Tr}\!\left[ \mathbf{F}_k^{\mathsf{H}} \mathbf{F}_{k'} \, \mathbb{E}\!\left[ \mathbf{a}_{k'} \mathbf{a}_k^{\mathsf{H}} \right] \right] \\
    &= \sum_{k=1}^{K} \sum_{k'=1}^{K} \text{Tr}\!\left[ \mathbf{F}_k^{\mathsf{H}} \mathbf{F}_{k'} \, \mathbf{I}_{N_S} \, \delta_{kk'} \right] \\
    &= \sum_{k=1}^{K} \text{Tr}\!\left[ \mathbf{F}_k^{\mathsf{H}} \mathbf{F}_{k} \right] \\
\end{align}
\end{subequations}
\end{comment}
...
The block-diagonal structure of $\mathbf{G}$ indicates that each \ac{UT} processes only its own received signal vector, i.e.\ no cooperation is assumed between the different \acp{UT}.
...
\label{eq:mi_su_mimo_gaussian_channel}
    I(\mathbf{x};\mathbf{y})
    = \log_2\left(\det\left(\mathbf{I}_{N_R} +  \mathbf{C}_{\mathrm{n}}^{-1} \; \mathbf{H}\,\mathbf{C}_{\mathrm{x}}\,\mathbf{H}^\mathsf{H} \right)\right).
\end{equation}
...
\begin{aligned}
    & C = \max_{\mathbf{C}_{\mathrm{x}}} \; \log_2\left(\det\left(\mathbf{I}_{N_R} + \mathbf{C}_{\mathrm{n}}^{-1} \; \mathbf{H}\,\mathbf{C}_{\mathrm{x}}\,\mathbf{H}^\mathsf{H} \right)\right) \\
...
The sum capacity corresponds to the special case where all weights are equal to one. The maximization in both cases is typically subject to the total transmit power constraint, as formulated in \eqref{eq:mu_mimo_power_constraint}.
...
In the uplink channel, all $K$ \acp{UT} transmit simultaneously, with \ac{UT}~$k$ sending a vector $\mathbf{x}_k^{\text{up}} \in \mathbb{C}^{N_R}$ to the \ac{BS}.
...
\label{eq:mi_mu_mimo_uplink_gaussian_channel}
    I(\mathbf{x}_1^{\text{up}}, \dots, \mathbf{x}_K^{\text{up}}; \mathbf{y})
    = \log_2\!\left(\det\!\left(\mathbf{I}_{N_T} + \mathbf{C}_{\mathrm{n}}^{-1} \sum_{k=1}^{K} \mathbf{H}_k\,\mathbf{C}_{\mathrm{x},\, k}^{\text{up}}\,\mathbf{H}_k^\mathsf{H} \right)\right).
\end{equation}
...
\begin{aligned}
    & C_{\text{sum}} = \max_{\mathbf{C}_{\mathrm{x},\,1}^{\text{up}}, \dots, \mathbf{C}_{\mathrm{x},\,K}^{\text{up}}} \; \log_2\!\left(\det\!\left(\mathbf{I}_{N_T} + \mathbf{C}_{\mathrm{n}}^{-1} \sum_{k=1}^{K} \mathbf{H}_k\,\mathbf{C}_{\mathrm{x},\, k}^{\text{up}}\,\mathbf{H}_k^\mathsf{H} \right)\right) \\
