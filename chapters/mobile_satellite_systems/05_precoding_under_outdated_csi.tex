...
How severely this mismatch degrades performance depends on how much the channel has evolved during the feedback delay $\tau_{\mathrm{CSI}}$.
...
Beyond the point $\tau_{\mathrm{CSI}} = T_c$, the channel available at the \ac{GW} and the channel actually experienced by the \acp{UT} are only weakly correlated such that the precoder is in effect designed for a channel that bears little resemblance to the true one.
...
\begin{figure}[!htb]
    ...
    \end{subfigure}
    \caption[Impact of outdated \acs{CSI} on the achievable \acs*{SR} (\acs{ZF} and \acs{BD} precoding)]{
...
\begin{figure}[!htb]
    ...
    \caption[Impact of outdated \acs{CSI} on the achievable \acs*{SR} (\acs{WMMSE} precoding)]{
...
Among the many possible prediction methods, a linear \acf{AR} model (see Appendix~\ref{appendix:ar_models}) is adopted here as a simple and tractable baseline. It leads to a closed-form predictor, requires only knowledge of the autocorrelation function, and entails a very limited computational complexity.
...
Merk op dat men evenwel kan kiezen om de kanaalvoorspelling volledig uit te voeren at the \ac{GW}. 
De keuze hangt af van de beschikbare rekenkracht van de toestellen en de gewenste architectuur van het systeem.
...
\end{equation}
Since the autocorrelation function is fully specified by the assumed channel model, these coefficients can be computed offline once the predictor order $p$, the pilot period $T_p$, and the delay $\tau_{\mathrm{CSI}}$ are determined.
...
Verder veronderstelt het kannalmodel ook dat het kanaal stationairy is, d.w.z. dat de statistieken van het kanaal niet veranderen in de tijd. Omdat de UT's beweeglijk zijn, kan deze aanname in de prijktijk niet altijd worden gegarandeerd. In dit geval wordt het AR-model vervangen door een adaptief AR-model (see Appendix~\ref{appendix:adaptive_ar_models}).
...
\begin{figure}[!b]
    ...
    \caption[Impact of the pilot rate on the achievable \acs*{SR} with \acs{AR}-based compensation]{
...
% pilot rate
For the first sweep, the context window length is fixed at $2\,T_c$. This is a generous choice, made to ensure that no potentially useful information would be excluded.
...
\begin{figure}[!htb]
    ...
    \caption[Impact of the window length on the achievable \acs*{SR} with \acs{AR}-based compensation]{
...
\begin{figure}[!htb]
    ...
    \end{subfigure}
    \caption[Impact of the AR-based compensation on achievable \acs*{SR} (\acs{ZF} and \acs{BD} precoding)]
    {
...
Only when $\tau_{\mathrm{CSI}}$ approaches $2\,T_c$, the residual prediction error is large enough to reintroduce an interference floor, and the familiar saturation behaviour as described in Section~\ref{section:outdated_csi_impact} reappears.
    ...
    \makebox[\textwidth][c]{%
        \begin{minipage}{1.2\textwidth}
            ...
            \begin{subfigure}{0.245\textwidth}
                ...
                \caption{$\tau_{\mathrm{CSI}} = \frac{1}{4} T^{\mathrm{NLoS}}_c$}
            \end{subfigure}
            \hfill
            \begin{subfigure}{0.245\textwidth}
                \centering
                ...
                \caption{$\tau_{\mathrm{CSI}} = \frac{2}{4} T^{\mathrm{NLoS}}_c$}
            \end{subfigure}
            \hfill
            \begin{subfigure}{0.245\textwidth}
                \centering
                ...
                \caption{$\tau_{\mathrm{CSI}} = \frac{3}{4} T^{\mathrm{NLoS}}_c$}
            \end{subfigure}
            \hfill
            \begin{subfigure}{0.245\textwidth}
                \centering
                ...
                \caption{$\tau_{\mathrm{CSI}} = \frac{4}{4} T^{\mathrm{NLoS}}_c$}
            \end{subfigure}
            ...
            \begin{subfigure}{0.245\textwidth}
                ...
                \caption{$\tau_{\mathrm{CSI}} = \frac{5}{4} T^{\mathrm{NLoS}}_c$}
            \end{subfigure}
            \hfill
            \begin{subfigure}{0.245\textwidth}
                \centering
                ...
                \caption{$\tau_{\mathrm{CSI}} = \frac{6}{4} T^{\mathrm{NLoS}}_c$}
            \end{subfigure}
            \hfill
            \begin{subfigure}{0.245\textwidth}
                \centering
                ...
                \caption{$\tau_{\mathrm{CSI}} = \frac{7}{4} T^{\mathrm{NLoS}}_c$}
            \end{subfigure}
            \hfill
            \begin{subfigure}{0.245\textwidth}
                \centering
                ...
                \caption{$\tau_{\mathrm{CSI}} = \frac{8}{4} T^{\mathrm{NLoS}}_c$}
            \end{subfigure}

        \end{minipage}
    }

    \caption[Impact of the AR-based compensation on achievable \acs*{SR} (\acs*{WMMSE} precoding)]
    {
        ...
        The compensation gain is strongest for $\tau_{\mathrm{CSI}} \leq T^{\mathrm{NLoS}}_c$ and decreases progressively as the feedback delay exceeds the channel coherence time.
    }
...
In principle, a predictor that exploits this spatial correlation jointly with the temporal correlation could yield more accurate predictions than the element-wise approach of Section~\ref{subsection:compensation_strategy}.
...
\begin{figure}[!htb]
    ...
    \end{subfigure}
    \caption[Impact of Outdated \acs{CSI} and Predicted \acs{CSI} on the \acs{BER} (\acs{ZF} precoding)]
...
\acp{MSS} typically operate in the $L$-band~\cite{1_signal_processing_for_satellites}, with carrier frequencies between $1$ and $2~\mathrm{GHz}$, and corresponding wavelengths ranging from $15$ to $30~\mathrm{cm}$.
...
\begin{table}[!htb]
    ...
    \begin{tabular}{>{\centering\arraybackslash}m{3.2cm}|>{\centering\arraybackslash}m{2.8cm}>{\centering\arraybackslash}m{2.8cm}}
        \hline
        $\text{UT velocity}$ & $f_D$ & $T_c^{\mathrm{NLoS}}$ \\
        \hline
