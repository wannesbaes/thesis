...
It is well-known that the Cholesky decomposition of a matrix $\mathbf{C}$ exists and is unique if and only if $\mathbf{C}$ is an Hermitian matrix and a positive-definite matrix, i.e.:
\begin{equation}
...
Consider a autocorrelation function $R_h(\tau)$ of a wide-sense stationary process, i.e. $R_h$ does not depend on the absolute time $t$, but only on the time difference $\tau$.
...
We consider a autocorrelation function $R_h(\tau)$ that corresponds to a power spectral density that makes physical sense, i.e. $S_h(f)$ is positive for all frequencies $f$ and strictly positive for a set of positive measure.
...
The underlying idea behind this method is that passing white complex Gaussian noise through a linear time-invariant filter with frequency response $G(f)$ yields an output process whose power spectral density is equal to $|G(f)|^2$.
